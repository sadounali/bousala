...
const StudentPortal: React.FC<StudentPortalProps> = ({ onBack, user: userProp }) => {
  const { t, i18n } = useTranslation();
  const { tier, user: authUser } = useAuth();
  // Prefer the Firebase user from AuthContext (has uid), fall back to prop
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {/* Student Profile Header — hidden for guests */}
      {isGuest ? (
        <div className="minimal-card p-8 bg-white flex flex-col items-center text-center gap-4">
          <div className="w-20 h-20 rounded-3xl bg-oued-blue/10 flex items-center justify-center">
            <GraduationCap className="w-10 h-10 text-oued-blue" />
          </div>
          <div>
            <h2 className="text-xl font-black text-slate-900 mb-1">
              {i18n.language === 'ar' ? 'مرحباً بك في بوصلة' : 'Welcome to Bousala'}
            </h2>
            <p className="text-sm text-slate-500 max-w-sm mx-auto">
              {i18n.language === 'ar'
                ? 'أنشئ حساباً مجانياً للوصول إلى جميع الميزات وتحليل مذكرتك.'
                : 'Create a free account to access all features and analyze your thesis.'}
            </p>
          </div>
        </div>
      ) : (
            ...
            <AnimatePresence mode="wait">
              {!isGuest && activeTab === 'pedagogical' ? (
                ...
                >
                  ...
                  {isGuest && (
                    <div className="flex flex-col items-center gap-4 p-8 rounded-2xl bg-oued-blue/5 border border-oued-blue/20 mb-2 text-center">
                      <div className="p-4 bg-oued-blue/10 rounded-2xl text-oued-blue">
                        <User className="w-8 h-8" />
                      </div>
                      <div>
                        <p className="text-lg font-black text-slate-800 mb-1">
                          {i18n.language === 'ar' ? 'أنشئ حساب طالب مجاناً' : 'Create a Student Account'}
                        </p>
                        <p className="text-sm text-slate-500 max-w-xs mx-auto">
                          {i18n.language === 'ar'
                            ? 'سجّل حساباً للوصول إلى تحليل المذكرات وسجل التقارير وجميع الميزات.'
                            : 'Sign up to access thesis analysis, report history, and all features.'}
                        </p>
                      </div>
                      <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs">
                        <button
                          onClick={onBack}
                          className="flex-1 py-3 bg-oued-blue text-white rounded-2xl text-sm font-bold hover:bg-oued-blue/90 transition-all shadow-lg shadow-oued-blue/20"
                        >
                          {i18n.language === 'ar' ? 'إنشاء حساب' : 'Sign Up'}
                        </button>
                        <button
                          onClick={onBack}
                          className="flex-1 py-3 bg-white border border-slate-200 text-slate-700 rounded-2xl text-sm font-bold hover:bg-slate-50 transition-all"
                        >

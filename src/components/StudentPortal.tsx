...
const StudentPortal: React.FC<StudentPortalProps> = ({ onBack, user, onLogin }) => {
            ...
            <AnimatePresence mode="wait">
              {!isGuest && activeTab === 'pedagogical' ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">
                        {i18n.language === 'ar' ? 'الاسم' : 'First Name'} <span className="text-red-500">*</span>
                      </label>
                      <div className="relative">
                        ...
                        <input
                          ...
                          placeholder={i18n.language === 'ar' ? 'مثال: محمد' : 'e.g. Mohamed'}
                          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:border-oued-gold focus:ring-0 text-sm transition-all"
                          required
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">
                        {i18n.language === 'ar' ? 'اللقب' : 'Last Name'} <span className="text-red-500">*</span>
                      </label>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input
                          ...
                          placeholder={i18n.language === 'ar' ? 'مثال: بن علي' : 'e.g. Ben Ali'}
                          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:border-oued-gold focus:ring-0 text-sm transition-all"
                          required
                        />
                      </div>
                ...
                >
                  ...
                  {isGuest && (
                    <div className="flex flex-col items-center gap-4 p-6 rounded-2xl bg-oued-blue/5 border border-oued-blue/20 mb-2 text-center">
                      <div className="p-3 bg-oued-blue/10 rounded-xl text-oued-blue">
                        <User className="w-6 h-6" />
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-800 mb-1">
                          {i18n.language === 'ar' ? 'أنشئ حساب طالب مجاناً' : 'Create a free student account'}
                        </p>
                        <p className="text-xs text-slate-500">
                          {i18n.language === 'ar'
                            ? 'سجل حسابك الآن لحفظ تاريخ تحليلاتك وسجل التقارير وجميع الميزات.'
                            : 'Sign in to access analyses history, reports, and all features.'}
                        </p>
                      </div>
                      <button
                        onClick={onLogin}
                        className="px-8 py-2.5 bg-oued-blue text-white rounded-2xl text-sm font-bold shadow-lg shadow-oued-blue/20 hover:scale-105 transition-all flex items-center gap-2"
                      >
                  ...
                  <div className="p-6 rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50 flex flex-col items-center justify-center text-center gap-3">
                    <div className="p-3 bg-slate-100 rounded-2xl text-slate-400">
                      <Lock className="w-6 h-6" />
                    </div>
                    <div>
                      <h4 className="text-base font-black text-slate-500 mb-1">
                        {i18n.language === 'ar' ? 'كشف الانتحال' : 'Plagiarism Detection'}
                        <span className="mr-2 text-sm">🔐</span>
                      </h4>
                      <p className="text-xs text-slate-400">
                        {i18n.language === 'ar' ? 'هذه الميزة قادمة قريباً — ستتمكن من فحص مذكرتك بحثاً عن الانتحال مباشرةً من بوصلة.' : 'This feature is coming soon — you'll be able to check your thesis for plagiarism directly from Compass.'}
                      </p>
                    </div>
                    <span className="px-4 py-1.5 bg-slate-200 text-slate-500 rounded-full text-[10px] font-bold uppercase tracking-widest">

...
function scoreFromCount(count: number, textLength: number = 5000): number {
  // Normalize by text length: longer texts naturally have more keyword occurrences
  const normalized = count / Math.max(1, textLength / 1000);
  // Scale: 0 = 0, 2+ per 1000 words = 5
  return Math.min(5, parseFloat((normalized * 2.5).toFixed(1)));
}
...
function buildSDGs(sdgIds: number[], text: string) {
  ...
  // Pre-compute max count across all active SDGs for relative scaling
  const counts = sdgIds.map(id => countKeywords(text, SDG_INFO[id].keywords));
  const maxCount = Math.max(1, ...counts);

  return sdgIds.map((id, idx) => {
    const info = SDG_INFO[id];
    const count = counts[idx];
    // Relative percentage: top SDG gets ~90%, others scale accordingly
    // Minimum 5% if it appears at all, 0% if count is 0
    const raw = count === 0 ? 0 : Math.round((count / maxCount) * 85 + 5);
    const percentage = Math.min(95, raw);
    const strength = percentage >= 65 ? "عالية" : percentage >= 35 ? "متوسطة" : "منخفضة";
...
function buildTargets(sdgId: number, text: string) {
  ...
  return entry.map(t => ({
    ...
    indicators: [{
      ...
      assessment: {
        measuresDirectly: countKeywords(text, SDG_INFO[sdgId].keywords) >= 3,
        suggestsSolutions: text.length > 2000,
        collectsData: text.includes("بيانات") || text.includes("إحصاء") || text.includes("قياس"),
        contributionScore: Math.min(5, Math.round(scoreFromCount(countKeywords(text, SDG_INFO[sdgId].keywords), text.length) + 1)),
...
function buildMetrics(text: string) {
  const len = text.length;
  return [
    { name: "البعد البيئي", score: scoreFromCount(countKeywords(text, METRIC_KEYWORDS.environmental), len), description: "مدى تناول الدراسة للقضايا البيئية والمناخية" },
    { name: "البعد الاجتماعي", score: scoreFromCount(countKeywords(text, METRIC_KEYWORDS.social), len), description: "مدى تناول الدراسة للقضايا الاجتماعية والإنسانية" },
    { name: "البعد الاقتصادي", score: scoreFromCount(countKeywords(text, METRIC_KEYWORDS.economic), len), description: "مدى تناول الدراسة للتنمية الاقتصادية" },
    { name: "الأهمية الجوهرية", score: scoreFromCount(countKeywords(text, METRIC_KEYWORDS.materiality), len), description: "أهمية المواضيع المطروحة بالنسبة للاستدامة" },
    { name: "قابلية التوسع", score: scoreFromCount(countKeywords(text, METRIC_KEYWORDS.scalability), len), description: "إمكانية تطبيق نتائج الدراسة على نطاق أوسع" },
  ];
}
...
function buildPillars(text: string, specialization: string) {
  ...
  const len = text.length;
  const e = Math.min(5, scoreFromCount(countKeywords(text, METRIC_KEYWORDS.environmental), len) * we);
  const s = Math.min(5, scoreFromCount(countKeywords(text, METRIC_KEYWORDS.social), len) * ws);
  const g = Math.min(5, scoreFromCount(countKeywords(text, METRIC_KEYWORDS.economic), len) * wg);
...
export async function analyzeSustainabilityDocument(
  ...
  const avgSdgPct = sdgs.length > 0 ? sdgs.reduce((s, g) => s + g.percentage, 0) / sdgs.length : 0;
  const sdgCoverage = (sdgIds.length / 17) * 100;
  // 50% from avg SDG alignment + 30% from metric scores + 20% from coverage breadth
  const overallScore = Math.min(95, Math.round(avgSdgPct * 0.5 + (avgMetric / 5) * 100 * 0.3 + sdgCoverage * 0.2));

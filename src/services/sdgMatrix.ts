...
function buildAnalysisResult(
  ...
  const avg = (ids: number[]) =>
    Math.round(ids.reduce((s, id) => s + (scores[id] || 0), 0) / ids.length / 20);

  const metrics = [
    { name: "البيئي",         score: avg(envSDGs),        description: "مدى ارتباط البحث بالأهداف البيئية (6، 7، 13، 14، 15)" },
    { name: "الاجتماعي",     score: avg(socSDGs),         description: "مدى ارتباط البحث بالأهداف الاجتماعية (1-5، 10، 11، 16)" },
    { name: "الاقتصادي",     score: avg(ecoSDGs),         description: "مدى ارتباط البحث بالأهداف الاقتصادية (8، 9، 12، 17)" },
  ...
  // ركائز ESG
  const pillars = [
    { name: "البيئي (E)", score: Math.round(avg(envSDGs) * 20), comment: "الأثر البيئي للبحث", notes: "يشمل الطاقة، المياه، المناخ، التنوع البيولوجي" },
    { name: "الاجتماعي (S)", score: Math.round(avg(socSDGs) * 20), comment: "الأثر الاجتماعي للبحث", notes: "يشمل التعليم، الصحة، المساواة، العدالة" },

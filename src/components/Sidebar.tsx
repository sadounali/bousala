...
import { Home, History, UserCog, BookOpen, ChevronRight, ChevronLeft, X, ShieldCheck } from 'lucide-react';
...
export default function Sidebar({ currentView, onViewChange, isOpen, onToggle, isMobile = false }: SidebarProps) {
  ...
  const allMenuItems = [
    { id: AppView.MAIN, label: t('nav.home'), icon: Home, guestAllowed: true },
    { id: AppView.THESIS, label: t('nav.thesis'), icon: BookOpen, guestAllowed: false },
    { id: AppView.HISTORY, label: t('nav.history'), icon: History, guestAllowed: false },
    { id: AppView.STUDENT_INFO, label: t('nav.profile'), icon: UserCog, guestAllowed: true },
  ];

  const menuItems = allMenuItems.filter(item => !isGuest || item.guestAllowed);
      ...
      <div className="flex-1 py-6 px-3 space-y-2">
        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon className={`w-5 h-5 min-w-[20px] ${isActive ? 'text-white' : 'group-hover:scale-110 transition-transform'}`} />
              {(isOpen || isMobile) && (

            ...
            {view === AppView.MAIN ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div 
                    onClick={() => tier === UserTier.GUEST ? setView(AppView.STUDENT_INFO) : setView(AppView.THESIS)}
                    className={`minimal-card p-8 bg-white border-oued-blue/5 hover:border-oued-blue transition-all cursor-pointer group relative ${tier === UserTier.GUEST ? 'opacity-60' : ''}`}
                  >
                    {tier === UserTier.GUEST && (
                      <div className="absolute top-3 end-3 bg-slate-200 text-slate-500 text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                        <Lock className="w-2.5 h-2.5" />{isAr ? 'للمسجّلين' : 'Sign in'}
                      </div>
                    )}
                  ...
                  <div 
                    onClick={() => tier === UserTier.GUEST ? setView(AppView.STUDENT_INFO) : setView(AppView.HISTORY)}
                    className={`minimal-card p-8 bg-white border-oued-gold/5 hover:border-oued-gold transition-all cursor-pointer group relative ${tier === UserTier.GUEST ? 'opacity-60' : ''}`}
                  >
                    {tier === UserTier.GUEST && (
                      <div className="absolute top-3 end-3 bg-slate-200 text-slate-500 text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                        <Lock className="w-2.5 h-2.5" />{isAr ? 'للمسجّلين' : 'Sign in'}
              ...
              >
                {tier === UserTier.GUEST ? (
                  ...
                  (() => { setView(AppView.STUDENT_INFO); return null; })()

...
export default function App() {
  ...
  // Auto-redirect to PORTAL when user logs in from AuthScreen
  useEffect(() => {
    if (user && view === AppView.PORTAL) {
      setView(AppView.PORTAL); // force re-render
    }
    // If user just logged in and we're still on LANDING, go to PORTAL
    if (user && view === AppView.LANDING && !authLoading) {
      setView(AppView.PORTAL);
    }
  }, [user, authLoading]);
          ...
          <AnimatePresence mode="wait">
            {view === AppView.MAIN ? (
              ...
              <motion.section
                ...
                exit={{ opacity: 0, y: -10 }}
              >
                <StudentPortal user={user} onBack={() => setView(AppView.MAIN)} onLogin={handleLogin} onStartAnalysis={() => setView(AppView.THESIS)} />
              </motion.section>
            ) : view === AppView.ADMIN ? (
              <motion.section
                key="admin"

...
const SDG_INFO: Record<number, { name: string; label: string; keywords: string[] }> = {
  1:  { name: "القضاء على الفقر", label: "SDG 1", keywords: ["فقر","دخل","اجتماعي","تنمية","توزيع"] },
  2:  { name: "القضاء على الجوع", label: "SDG 2", keywords: ["غذاء","زراعة","أمن غذائي","إنتاج","محصول","تغذية"] },
  3:  { name: "الصحة الجيدة والرفاه", label: "SDG 3", keywords: ["صحة","طب","مرض","علاج","وقاية","رفاه","بيئة صحية"] },
  4:  { name: "التعليم الجيد", label: "SDG 4", keywords: ["تعليم","تدريب","تعلم","مهارة","مدرسة","جامعة","معرفة"] },
  5:  { name: "المساواة بين الجنسين", label: "SDG 5", keywords: ["مرأة","جنس","مساواة","تمكين","نوع اجتماعي"] },
  6:  { name: "المياه النظيفة والصرف الصحي", label: "SDG 6", keywords: ["مياه","ماء","صرف","نظافة","ري","تلوث مائي"] },
  7:  { name: "طاقة نظيفة وبأسعار معقولة", label: "SDG 7", keywords: ["طاقة","كهرباء","متجددة","شمسية","رياح","وقود"] },
  8:  { name: "العمل اللائق ونمو الاقتصاد", label: "SDG 8", keywords: ["عمل","اقتصاد","نمو","توظيف","إنتاجية","تشغيل"] },
  9:  { name: "الصناعة والابتكار والبنية التحتية", label: "SDG 9", keywords: ["صناعة","ابتكار","تقنية","بنية","تحتية","بحث","تطوير"] },
  10: { name: "الحد من أوجه عدم المساواة", label: "SDG 10", keywords: ["مساواة","عدالة","فجوة","توزيع","إدماج"] },
  11: { name: "مدن ومجتمعات مستدامة", label: "SDG 11", keywords: ["مدينة","عمران","سكن","نقل","تخطيط","مستدام"] },
  12: { name: "الاستهلاك والإنتاج المسؤولان", label: "SDG 12", keywords: ["استهلاك","إنتاج","نفايات","إعادة تدوير","كفاءة"] },
  13: { name: "العمل المناخي", label: "SDG 13", keywords: ["مناخ","انبعاثات","احترار","كربون","بيئة","تغير مناخي"] },
  14: { name: "الحياة تحت الماء", label: "SDG 14", keywords: ["بحر","محيط","أسماك","مائي","بحري","تلوث بحري"] },
  15: { name: "الحياة في البر", label: "SDG 15", keywords: ["غابة","تنوع حيوي","أرض","نبات","حيوان","بيئة برية"] },
  16: { name: "السلام والعدل والمؤسسات القوية", label: "SDG 16", keywords: ["قانون","عدالة","حوكمة","مؤسسة","حقوق","سلام"] },
  17: { name: "عقد الشراكات لتحقيق الأهداف", label: "SDG 17", keywords: ["شراكة","تعاون","دولي","تمويل","تكنولوجيا","بيانات"] },
};

// ========== الكلمات المفتاحية للمقاييس ==========

const METRIC_KEYWORDS = {
  environmental: ["بيئة","مناخ","طاقة","مياه","تلوث","انبعاثات","غابة","تنوع","أرض","كربون","متجددة","نفايات"],
  social: ["مجتمع","صحة","تعليم","عدالة","مساواة","فقر","مرأة","حقوق","سكن","أمن","رفاه"],
  economic: ["اقتصاد","تنمية","إنتاج","عمل","توظيف","نمو","تجارة","استثمار","دخل","ثروة"],
  materiality: ["أهمية","جوهري","محوري","أساسي","رئيسي","حيوي","ضروري","استراتيجي"],
  scalability: ["توسع","تطوير","نشر","تطبيق","ترقية","انتشار","عالمي","إقليمي","وطني"],
};
...
export async function analyzeSustainabilityDocument(
  ...
  // تحديد أهداف SDG
  const sdgIds = getSDGsForSpecialization(specialization, text);
  onProgress?.('sdg_mapping', 50);
  await new Promise(r => setTimeout(r, 300));

  // بناء المقاييس
  const metrics = buildMetrics(text);
  const pillars = buildPillars(text);
  onProgress?.('detailed_matrix', 70);
  await new Promise(r => setTimeout(r, 300));

  // بناء SDGs
  const sdgs = buildSDGs(sdgIds, text);
  ...
  const swot = buildSWOT(text, sdgIds, specialization);
